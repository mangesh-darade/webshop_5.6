...
class Pos_model extends CI_Model {
    ...
    // public function openRegister($data, $data_coinage, $data_coinage_transactions) {
    //     // Start the transaction
    //     $this->db->trans_start(); 
    //     echo '<pre>';
    //     print_r( $data_coinage);
    //     echo '</pre>';
    ...
    //         foreach ($data_coinage as $coinage) {
    ...
    //                     }
    //                 }
    //                 $type = $data_coinage_transactions[0]['type'];
    ...
    if (!empty($data_coinage)) {
        ...
        foreach ($data_coinage as $coinage) {
            ...
            if ($query_coinage->num_rows() > 0) {
                ...
                // Transaction type (assumes consistent type across all)
                $type = $data_coinage_transactions[0]['type'];
...
//                $this->db->group_by('date(date)');
    ...
    public function getTableGroup($tableids) {
        ...
        if (!empty($tableids)) {
            $tables = explode(",", $tableids);
            // Sanitize and quote the IDs for SQL
            $Ids = array_map('intval', $tables); 
            $idList = implode(",", $Ids);
            $whereClause = "WHERE id IN ($idList)";
        }
        $sql = "
            SELECT table_group 
            FROM sma_restaurant_tables 
            $whereClause
            GROUP BY table_group 
            ORDER BY CAST(SUBSTRING_INDEX(table_group, ' ', -1) AS UNSIGNED) ASC
        ";
        $query = $this->db->query($sql);
        $getTables = $query->result_array();
        return (!empty($getTables) ? $getTables : FALSE);
    }
   ...
   public function getPreviousPosSale() {
    ...
    public function update_register($data, $data_coinage, $data_coinage_transactions,$Register_id) {
        // Start the transaction
        // $this->db->trans_start(); 
        // Insert data into pos_register table
        if ($Register_id) {
        ...
        if (!empty($data_coinage)) {
            ...
            foreach ($data_coinage as $coinage) {
                ...
                if ($query_coinage->num_rows() > 0) {
                    ...
                    $transaction_type = $data_coinage_transactions[0]['transaction_type'];
                    // Prepare transaction data for insertion
                    $transaction_data[] = [
                        'coinage_id' => $coinage_id, 
                        'currency_value' => $coinage['currency_value'],
